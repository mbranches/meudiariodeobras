...
""")
    ...
    WHERE r.obraId = :obraId
      ...
      AND t.ativo IS TRUE
""")
    List<RelatorioDetailsProjection> findAllDetailsWithoutPdfLinkByObraId(Long obraId);
...
""")
    ...
    FROM RelatorioEntity r
        ...
        JOIN UserEntity u ON u.id = r.enversCreator
        JOIN UserEntity u2 ON u2.id = r.enversModifier
    WHERE r.obraId IN :obrasIds
      AND r.tenantId = :tenantId
      AND r.ativo IS TRUE
      AND t.ativo IS TRUE
    """)
    List<RelatorioDetailsProjection> findAllDetailsWithoutPdfUrlByObraIdInAndTenantId(HashSet<Long> obrasIds, Long tenantId);
